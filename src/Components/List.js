    ...
    let showThis=(filter==="Done")? true:(filter==="Undone")? false:"All"
      return (
        <div className="List">
          ...
          <ul>
          {
            todos.filter(c=>(showThis==="All")?c:(showThis===c.isDone)).map((c,i)=><Task c={c} key={i}/>)
          } 
          </ul>
          {(!todos.length)?"Your list is empty, nothing to do yet! ": <button id="ClearAll" onClick={()=>dispatch(clearAll())}>Clear All</button>}
